...
from ndoc.features import tech, docs, map, graph, link, log, verify, fix, test, module, build, doctor, hooks, todo, watch, ide, syntax, injector
...
def main():
    ...
    # Watch
    parser_watch = subparsers.add_parser("watch", help="Start live context daemon")
    parser_watch.set_defaults(func=lambda args: run_auto(watch.cmd_watch))

    # IDE
    parser_ide = subparsers.add_parser("ide", help="IDE integration")
    ide_subs = parser_ide.add_subparsers(dest="subcommand", help="IDE subcommands")
    ide_setup = ide_subs.add_parser("setup", help="Generate VS Code configuration")
    ide_setup.set_defaults(func=lambda args: run_auto(ide.cmd_setup))

    # Rules
    parser_rules = subparsers.add_parser("rules", help="Manage project rules")
    rules_subs = parser_rules.add_subparsers(dest="subcommand", help="Rules subcommands")
    rules_update = rules_subs.add_parser("update", help="Update _RULES.md from code")

    def handle_rules_update(root):
        inj = injector.UniversalInjector(root)
        inj.register("RULES", injector.collect_rules)
        # Update _RULES.md
        rules_path = root / "_RULES.md"
        if not rules_path.exists():
            # Auto-create if missing (User friendly)
            console.warning("_RULES.md not found. Creating it...")
            rules_path.write_text("# Project Rules\n\n## Code Rules\n<!-- NIKI_AUTO_RULES_START -->\n<!-- NIKI_AUTO_RULES_END -->\n", encoding='utf-8')
        
        if not inj.update_file(rules_path):
            console.info("No changes needed for _RULES.md")

    rules_update.set_defaults(func=lambda args: run_auto(handle_rules_update))
    ...
    # Hooks
    parser_hooks = subparsers.add_parser("hooks", help="Manage git hooks")
    hooks_subs = parser_hooks.add_subparsers(dest="subcommand", help="Hooks subcommands")
    hooks_install = hooks_subs.add_parser("install", help="Install git hooks")
    hooks_install.set_defaults(func=lambda args: run_auto(lambda root: hooks.cmd_hooks(root, "install")))

    # Todo
    parser_todo = subparsers.add_parser("todo", help="Manage code todos")
    todo_subs = parser_todo.add_subparsers(dest="subcommand", help="Todo subcommands")
    todo_update = todo_subs.add_parser("update", help="Scan code and update _NEXT.md")
    todo_update.set_defaults(func=lambda args: run_auto(lambda root: todo.cmd_todo(root, "update")))
