...
def check_coding_standards(root, active_languages):
    ...
    # 1. Collect all files to scan using centralized scanner
    # We scan src, engine, client, scripts
    target_dirs = ['engine', 'client', 'src', 'scripts']
    # Scanner will handle ignores
    
    # 2. Iterate and check
    target_dirs_exist = [d for d in target_dirs if (root / d).exists()]
    
    for file_path in scanner.walk_project_files(root):
        # Filter for specific target directories if any exist
        if target_dirs_exist:
            is_in_target = False
            try:
                # Check if file starts with any of the target directories relative to root
