...
class CapabilityManager:
    ...
    def ensure_package(cls, package_name: str, auto_install: bool = True) -> bool:
        ...
        print(f"--> [Capability] Detected need for optional package '{pypi_name}'.")

        if not auto_install:
            try:
                response = input(f"--> Install {pypi_name} now? [Y/n] ").strip().lower()
                if response not in ('', 'y', 'yes'):
                    print(f"--> Skipping {pypi_name} installation.")
                    return False
            except EOFError:
                print(f"--> Non-interactive mode. Skipping {pypi_name}.")
                return False
